...
class AssetCompute {
    ...
    async register() {
        ...
        if (!response.ok) {
            ...
            const error = new Error(`Unable to invoke /register: ${response.status} ${msg}`);
            if (response.status === 429) {
                error.retryAfter = parseInt(response.headers.get('retry-after')); 
            }
            throw error;
        } else {
    ...
    async unregister() {
        ...
        if (!response.ok) {
            const msg = await response.text();
            const error = new Error(`Unable to invoke /unregister: ${response.status}: ${msg} (details: ${JSON.stringify(response)})`);
            if (response.status === 429) {
                error.retryAfter = parseInt(response.headers.get('retry-after')); 
            }
            throw error;
    ...
    async process(source, renditions, userData) {
        ...
        if (!response.ok) {
            const msg = await response.text();
            const error = new Error(`Unable to invoke /process: ${response.status} ${msg}`);
            if (response.status === 429) {
                error.retryAfter = parseInt(response.headers.get('retry-after')); 
            }
            throw error;
