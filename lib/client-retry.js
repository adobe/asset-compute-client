...
class AssetComputeClientRetry extends AssetComputeClient {
    ...
    retryWaitTime(e) {
        ...
        let waitTime = Math.floor(Math.random() * (MAX_RETRY_WAIT_TIME - MIN_RETRY_WAIT_TIME + 1)) + MIN_RETRY_WAIT_TIME;

        // in seconds amount of time to wait before retrying the request
    ...
    async register() {
        ...
        if (!non429Response) {
            throw new Error("Running into 429s. Will not continue testing. Try re-running in a few minutes.");
