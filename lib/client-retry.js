...
class AssetComputeClientWithRetry extends AssetComputeClient {
    ...
    async register() {
        ...
        let retryCount = 0;
        while (this.shouldRetry(non429Response, retryCount)) {
            retryCount++;
            try {
                response = await super.register();
                non429Response = true;
            } catch (e) {
                // string comparision instead of `instanceof` since `instanceof` causes issues
                if (e.code === TOO_MANY_REQUEST_ERROR_CODE && e.name === TOO_MANY_REQUEST_ERROR) {
                    non429Response = false;
                    console.log(colors.red(`encountered a 429: ${e.message}.`));
                    const waitTime = this.retryWaitTime(e.retryAfter);
